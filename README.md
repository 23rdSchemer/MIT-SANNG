...
<b>UPDATE:  as of the current update to version 1.0, there are three versions of this software available.  One for Guile Scheme(github.com/23rdSchemer/SANNG-Guile-Edition), one for MIT-Scheme(github.com/23rdSchemer/MIT_SANNG) and one for Racket(planet.racket-lang.org/display.ss?package=sanng.plt&owner=23rdSchemer).  They all should work more or less identicaly.  The only differences being in their random number generators and in the function "vector-map" which is unique to Racket
