...
(define (connect-node input)
	(let rec ((vec (make-vector (vector-length input) 0)) (count 0) (len (vector-length input)))
		(if (eq? count len) vec
			(begin (vector-set! vec count (random 1.0))
				(rec vec (+ count 1) len)))))
	...
	(let rec ((in input-list) (out output-list) (net network) (ideal-MSE initial-ideal-MSE) (count 0) (count2 0))
		(let ((trained (train-net net (car in) (car out) ideal-MSE)))
			(cond ((eq? (cdr in) '()) trained)
				((and (eq? count2 0) (eq? (vector-ref trained 0) (vector-ref network 0)) (eq? count 10)) (display "Error, current network configuration will not converge") (newline))
...
(define (normalize-vec vec)
	(let ((m (apply max (vector->list vec))) (n (apply min (vector->list vec))))
		(map1 (lambda (x) (normalize x m n 1.0 0.0)) vec (vector-length vec) 0)))
